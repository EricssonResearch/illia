...
class TestLSTM:
    ...
    def test_init(self, lstm_fixture: tuple[LSTM, torch.Tensor]) -> None:
        ...
        assert hasattr(model, 'wf_distribution'), "LSTM must have forget gate weight distribution"
    ...
    def test_forward(self, lstm_fixture: tuple[LSTM, torch.Tensor]) -> None:
        ...
        assert isinstance(outputs, torch.Tensor), f"Incorrect outputs class, expected {torch.Tensor} and got {type(outputs)}"
        assert isinstance(hidden_state, torch.Tensor), f"Incorrect hidden_state class, expected {torch.Tensor} and got {type(hidden_state)}"
        assert isinstance(cell_state, torch.Tensor), f"Incorrect cell_state class, expected {torch.Tensor} and got {type(cell_state)}"

        # Check output shapes
        batch_size = inputs.shape[0]
        assert outputs.shape[0] == batch_size, f"Expected batch size {batch_size}, got {outputs.shape[0]}"
        assert outputs.shape[1] == model.output_size, f"Expected output size {model.output_size}, got {outputs.shape[1]}"
        assert hidden_state.shape == (batch_size, model.hidden_size), f"Expected hidden state shape {(batch_size, model.hidden_size)}, got {hidden_state.shape}"
        assert cell_state.shape == (batch_size, model.hidden_size), f"Expected cell state shape {(batch_size, model.hidden_size)}, got {cell_state.shape}"
    ...
    def test_backward(self, lstm_fixture: tuple[LSTM, torch.Tensor]) -> None:
        ...
        for name, parameter in model.embedding.named_parameters():
            if parameter.requires_grad:
                assert parameter.grad is not None, f"Incorrect backward computation, gradient of embedding.{name} shouldn't be None"
    ...
    def test_kl_cost(self, lstm_fixture: tuple[LSTM, torch.Tensor]) -> None:
        ...
        # Check type of outputs
        assert isinstance(kl_cost, torch.Tensor), f"Incorrect KL cost type, expected {torch.Tensor} and got {type(kl_cost)}"
        assert isinstance(num_params, int), f"Incorrect num_params type, expected {int} and got {type(num_params)}"

        # Check shape of kl cost (should be scalar)
        assert kl_cost.shape == (), f"Incorrect shape of KL cost, expected () and got {kl_cost.shape}"
    ...
    def test_jit(self, lstm_fixture: tuple[LSTM, torch.Tensor]) -> None:
        ...
        assert not torch.allclose(outputs_first, outputs_second, 1e-8), "Incorrect default freeze with torchscript."
        ...
        assert torch.allclose(outputs_first, outputs_second, 1e-8), "Incorrect freezing with torchscript."
        ...
        assert not torch.allclose(outputs_first, outputs_second, 1e-8), "Incorrect unfreezing with torchscript."
