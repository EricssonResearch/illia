...
conv_test_keys = [
    ...
    "weights_distribution", "bias_distribution",
    ]
...
def test_conv_freeze(
    ...
    input = tf.random.stateless_uniform(
    ...
    assert output_2.numpy() != pytest.approx(output_1.numpy(), rel=1e-6, nan_ok=False), \
...
def test_conv_forward(
    ...
    assert output.numpy() == pytest.approx(compiled_output.numpy(), rel=1e-8, nan_ok=False), \
